...
//  5. champs (numéro de page, nombre de pages) : la bibliothèque écrit tout le champ dans UN SEUL run
//     (début, instruction, séparateur, fin) et sans résultat. Word s'en accommode ; Word lui-même écrit
//     un run par élément avec un résultat en cache, la forme que les autres logiciels ont l'habitude de lire.
//     On la reproduit (Word n'affiche jamais ce résultat en cache : il recalcule ces champs).
...
// ---- 5. champs au format de Word ------------------------------------------------------------
// run SANS run imbriqué (une image peut contenir une zone de texte, donc d'autres runs : on n'y touche pas)
const RUN_SIMPLE = /<w:r(\s[^>]*)?>((?:(?!<w:r[\s>])[\s\S])*?)<\/w:r>/g;
const ENFANT_RUN = /<(w:[A-Za-z]+)\b(?:[^>"']|"[^"]*"|'[^']*')*?(?:\/>|>[\s\S]*?<\/\1>)/g;
const CACHE_CHAMP = { PAGE: '1', NUMPAGES: '1', SECTIONPAGES: '1' };
function normaliserChamps(xml) {
  return xml.replace(RUN_SIMPLE, (tout, attrs, contenu) => {
    if (!/<w:fldChar\b|<w:instrText\b/.test(contenu) || /<w:(drawing|pict|object)\b|<mc:|<w:rPrChange\b/.test(contenu)) return tout;
    const rpr = (contenu.match(/^<w:rPr>[\s\S]*?<\/w:rPr>/) || [''])[0];
    const reste = contenu.slice(rpr.length);
    const enfants = reste.match(ENFANT_RUN) || [];
    if (enfants.length < 2 || enfants.join('') !== reste) return tout;   // déjà découpé, ou structure imprévue
    const ouvre = '<w:r' + (attrs || '') + '>' + rpr;
    const out = [];
    let instr = '';
    enfants.forEach((e, i) => {
      out.push(ouvre + e + '</w:r>');
      if (/^<w:fldChar\b[^>]*w:fldCharType="begin"/.test(e)) instr = '';
      else if (/^<w:instrText\b/.test(e)) instr += e.replace(/<[^>]+>/g, '');
      else if (/^<w:fldChar\b[^>]*w:fldCharType="separate"/.test(e) && /^<w:fldChar\b[^>]*w:fldCharType="end"/.test(enfants[i + 1] || '')) {
        const cache = CACHE_CHAMP[(instr.trim().split(/\s+/)[0] || '').toUpperCase()];
        if (cache) out.push(ouvre + '<w:t>' + cache + '</w:t></w:r>');
      }
    });
    return out.join('');
  });
...
async function rendreDocxPortable(buf) {
  try {
    ...
    for (const nom of Object.keys(zip.files)) {
      ...
      else if (PARTIES_TEXTE.test(nom)) zip.file(nom, normaliserChamps(retoucherPartie(await f.async('string'), { fusion: true, tableaux: true })));
    }
...
module.exports = { rendreDocxPortable, retoucherPartie, retoucherStyles, normaliserChamps };
