// Article « Financer une formation en langue en 2026 » + ses trois posts LinkedIn.
// Tous les faits proviennent de la recherche sourcée du 03/08/2026 : chaque chiffre a son texte
// et sa date. Les formulations prudentes imposées par le tri sont reprises telles quelles.
module.exports = {
  slug: 'financer-formation-langue-2026-ce-qui-a-change',
  titre: 'Financer une formation en langue en 2026 : ce qui a changé',
  categorie: 'Financement',
  motCle: 'financer une formation en langue en 2026',
  titreSeo: 'Financer une formation en langue en 2026 : le point',
  metaDescription: 'Plafond CPF de 1 500 €, participation portée à 150 €, et une exception réservée aux langues : le point complet sur les règles entrées en vigueur en 2026.',
  chapo: 'Un plafond, une participation relevée, et une exception qui ne concerne que les certifications de langue. Voici les règles réellement entrées en vigueur cette année, texte à l’appui.',
  altA: 'Répartition du financement d’une formation en langue entre droits CPF et financeurs complémentaires',
  altB: 'Les dates d’entrée en vigueur des règles de financement en 2026',

  corps: `      <p>Trois textes ont modifié en 2026 la façon de <strong>financer une formation en langue en 2026</strong>. Deux resserrent les règles. Le troisième, passé beaucoup plus inaperçu, ouvre au contraire une porte que les langues sont seules à emprunter.</p>
      <p>Nous les avons repris un par un, avec leur date d’entrée en vigueur et le texte qui les porte. Rien ici ne relève de l’annonce : tout est applicable aujourd’hui.</p>

      <h2>Un plafond de 1 500 € sur vos droits CPF</h2>
      <p>Depuis le <strong>26 février 2026</strong>, une formation qui prépare une certification enregistrée au <strong>Répertoire spécifique</strong> peut mobiliser <strong>1 500 € de droits CPF au maximum</strong>. Les certifications de langue relèvent de ce répertoire : le TOEIC, par exemple, y figure sous une fiche active.</p>
      <p>Une précision qui évite le contresens le plus coûteux : <strong>ce plafond porte sur vos droits, pas sur le prix de la formation</strong>. Une formation plus chère reste possible. Au-delà du plafond, l’administration indique que « le reste sera pris en charge par le bénéficiaire ou par un financeur tiers (l’employeur si vous êtes salarié, l’État, la région, un opérateur de compétences – Opco…) ».</p>
      <p>Le montant exact mobilisable pour votre dossier s’affiche dans votre parcours d’achat sur moncompteformation.gouv.fr. Nous vous invitons à le regarder avant de vous engager, pas après.</p>

{{ILLU_A}}

      <p>Deux catégories échappent à ce plafond : les formations préparant une certification enregistrée au <strong>RNCP</strong>, et la certification <strong>CléA</strong>. Attention toutefois : CléA porte sur la communication en français, pas sur les langues étrangères.</p>

      <h2>La participation est passée à 150 €</h2>
      <p>Depuis le <strong>2 avril 2026</strong>, toute inscription sur Mon Compte Formation suppose une participation forfaitaire de <strong>150 €</strong> réglée par le titulaire, contre 103,20 € auparavant. Les dossiers dont la demande a été déposée avant cette date restent au montant antérieur.</p>
      <p>Cette participation n’est due ni par les demandeurs d’emploi, ni par le salarié dont la formation fait l’objet d’un abondement de son employeur. Deux autres cas de dispense existent : la mobilisation des points du compte professionnel de prévention, et l’abondement au titre d’un accident du travail ou d’une maladie professionnelle.</p>
      <p>Un détail qui compte pour la suite : le décret fixe ce montant <em>pour l’année 2026</em>. Le montant applicable en 2027 sera arrêté séparément, indexé sur l’évolution des prix.</p>

      <h2>L’exception réservée aux langues</h2>
      <p>C’est le changement le moins commenté, et le plus favorable à qui apprend une langue.</p>
      <p>Depuis le <strong>27 juin 2026</strong>, le CPF ne peut plus financer une certification que vous avez <strong>déjà obtenue</strong>. La loi prévoit une seule exception, et elle vise expressément « une certification visant à atteindre un niveau de connaissance d’une langue ».</p>
      <p>Autrement dit : repasser un test de langue pour attester d’une progression reste finançable. Un B1 obtenu il y a deux ans n’interdit pas de financer le passage visant le B2. C’est cohérent avec la nature même d’une certification de niveau, qui photographie un état à un instant donné et non un acquis définitif.</p>

{{ILLU_B}}

      <h2>Ne pas se présenter à l’examen a désormais un coût</h2>
      <p>Le même texte a introduit un garde-fou. Depuis le 27 juin 2026, si vous ne vous présentez pas aux épreuves sans motif légitime, vous ne pouvez plus mobiliser vos droits pour régler l’organisme, et la Caisse des dépôts peut vous demander le remboursement des sommes déjà utilisées.</p>
      <p>Ce que recouvre exactement un « motif légitime » n’est pas encore défini : le décret d’application n’était pas publié début août 2026. Nous nous garderons donc d’en dresser la liste. En pratique, la règle à retenir est simple : la date d’examen se cale au moment de l’inscription, pas à la fin du parcours.</p>

      <h2>Côté entreprise : la fin du paiement direct par les OPCO</h2>
      <p>Un changement distinct, souvent confondu avec le précédent. À la suite de l’assujettissement des OPCO à la TVA, <strong>neuf OPCO ont annoncé qu’ils cesseraient le paiement direct des organismes de formation au 1<sup>er</sup> octobre 2026</strong>. L’entreprise avance alors le coût et se fait rembourser ensuite.</p>
      <p>Deux précisions, parce que beaucoup d’articles se trompent sur ce point. Aucun texte ne supprime la subrogation : elle reste juridiquement possible, et l’arrêt du paiement direct est une décision propre à chaque OPCO. Les dates de bascule et les exceptions varient donc : plusieurs OPCO annoncent maintenir le paiement direct pour le plan de développement des compétences des entreprises de moins de 50 salariés. <strong>Vérifiez auprès du vôtre.</strong></p>
      <p>Enfin, l’effet sur votre trésorerie dépend du régime de TVA de votre organisme de formation. Un organisme exonéré facture sans TVA : il n’y a alors rien à avancer ni à récupérer à ce titre.</p>

      <h2>Ce qui ne change pas, et ce qui n’a jamais existé</h2>
      <p>La <strong>période de reconversion</strong>, entrée en vigueur le 1<sup>er</sup> février 2026, remplace la Pro-A et Transitions collectives. Elle vise l’acquisition d’une certification du RNCP, d’un CQP, d’un ou plusieurs blocs de compétences, ou du socle CléA. Une certification de langue seule, enregistrée au Répertoire spécifique, <strong>n’entre donc pas dans ce cadre</strong>. Une formation en langue peut en revanche y trouver sa place lorsqu’elle prépare un bloc de compétences linguistique d’une certification RNCP, ou qu’elle est une composante d’un parcours certifiant.</p>
      <p>Plus aucune <strong>Pro-A</strong> ne peut être conclue depuis le 1<sup>er</sup> janvier 2026. Les avenants signés avant cette date restent régis par l’ancien droit jusqu’à leur terme.</p>
      <p>Pour les demandeurs d’emploi, l’<strong>aide individuelle à la formation</strong> de France Travail n’est pas un droit : elle est étudiée dossier par dossier, sur la base du devis et du projet, sans barème public. Méfiez-vous des plafonds chiffrés qui circulent : nous n’en avons trouvé aucun dans une source officielle.</p>

      <h2>Comment nous procédons</h2>
      <p>Nous regardons votre solde et le coût réel du parcours <em>avant</em> de vous annoncer quoi que ce soit, puis nous montons le dossier de cofinancement avec vous. Depuis février 2026, dire d’une formation en langue qu’elle est « intégralement prise en charge » est rarement exact, et nous ne le dirons pas sans l’avoir vérifié.</p>
      <p>Nos <a href="/formations.html">formats de formation</a> et nos <a href="/financement.html">dispositifs de financement</a> détaillent le reste. Si le plafond vous concerne, notre article sur <a href="/blog/cpf-plafonne-financer-le-reste-formation-langue">le reste à charge CPF</a> passe en revue les financeurs qui prennent le relais, et celui sur <a href="/blog/certification-anglais-finance-cpf-comment-verifier">la vérification d’une certification</a> explique comment contrôler vous-même son enregistrement.</p>
      <p>Une question sur votre situation ? <a href="/contact.html">Écrivez-nous</a> avant d’engager la moindre démarche.</p>`,

  faq: [
    { q: 'Le plafond de 1 500 € s’applique-t-il à toutes les formations ?', r: 'Non. Il vise les formations sanctionnées par une certification enregistrée au Répertoire spécifique, ce qui est le cas des certifications de langue. Les formations préparant une certification du RNCP ne sont pas concernées, et la certification CléA en est expressément exclue.' },
    { q: 'Le plafond limite-t-il le prix de ma formation ?', r: 'Non, il limite la part payée par vos droits CPF. Une formation plus chère reste possible : au-delà du plafond, le reste est pris en charge par vous-même ou par un financeur tiers, employeur, État, région ou opérateur de compétences. Le montant exact mobilisable pour votre dossier s’affiche dans votre parcours d’achat sur moncompteformation.gouv.fr.' },
    { q: 'Puis-je financer un test de langue que j’ai déjà passé ?', r: 'Oui. Depuis le 27 juin 2026, le CPF ne finance plus une certification déjà obtenue, mais la loi réserve expressément le cas des certifications de niveau de langue. Repasser un test pour attester d’une progression reste donc finançable.' },
    { q: 'Dois-je payer les 150 € de participation ?', r: 'Pas dans tous les cas. Les demandeurs d’emploi en sont dispensés, ainsi que les salariés dont la formation fait l’objet d’un abondement de leur employeur. La mobilisation des points du compte professionnel de prévention et l’abondement au titre d’un accident du travail ou d’une maladie professionnelle dispensent également de cette participation.' },
    { q: 'Que se passe-t-il si je ne me présente pas à l’examen ?', r: 'Depuis le 27 juin 2026, une absence sans motif légitime empêche de mobiliser vos droits pour régler l’organisme, et la Caisse des dépôts peut demander le remboursement des sommes déjà utilisées. Le décret précisant ce qu’est un motif légitime n’était pas publié début août 2026.' },
    { q: 'Ma formation en langue peut-elle entrer dans une période de reconversion ?', r: 'Pas seule. La période de reconversion vise une certification du RNCP, un CQP, un ou plusieurs blocs de compétences ou le socle CléA. Une certification de langue enregistrée au Répertoire spécifique n’entre pas dans ce cadre, mais une formation en langue peut préparer un bloc de compétences linguistique d’une certification RNCP ou s’insérer dans un parcours certifiant. À examiner avec votre OPCO.' }
  ],

  sources: [
    { titre: 'Article D. 6323-1 A du code du travail : plafonds de droits CPF', url: 'https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000053569786' },
    { titre: 'Décret n° 2026-234 du 30 mars 2026 : participation obligatoire', url: 'https://www.legifrance.gouv.fr/jorf/id/JORFTEXT000053742996' },
    { titre: 'Article L. 6323-6 du code du travail : exception pour les certifications de langue', url: 'https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000054337187' },
    { titre: 'Article L. 6323-7 du code du travail : dispenses de participation', url: 'https://code.travail.gouv.fr/code-du-travail/l6323-7' },
    { titre: 'Service-Public : quelles évolutions en 2026 pour l’utilisation du CPF', url: 'https://www.service-public.gouv.fr/particuliers/actualites/A18831' },
    { titre: 'Article L. 6324-1 du code du travail : période de reconversion', url: 'https://www.legifrance.gouv.fr/codes/article_lc/LEGIARTI000038951696/' },
    { titre: 'France Compétences : répertoire spécifique (recherche officielle)', url: 'https://www.francecompetences.fr/recherche/rs/' }
  ],

  postsLi: [
    { angle: 'La question', texte:
`« Mon TOEIC date de deux ans. Je peux le repasser avec mon CPF ? »

Oui, et c'est une exception que les langues sont seules à avoir. Elle change ce qu'on peut espérer quand on cherche à financer une formation en langue en 2026.

Depuis le 27 juin 2026, le CPF ne finance plus une certification déjà obtenue. La loi réserve un seul cas : « une certification visant à atteindre un niveau de connaissance d'une langue ».

Autrement dit, un B1 obtenu il y a deux ans n'interdit pas de financer le passage visant le B2.

C'est logique : une certification de niveau photographie un état à un instant donné, pas un acquis définitif.

Ce point est passé largement inaperçu. Il change pourtant la donne pour qui veut faire reconnaître sa progression.

Nous avons fait le point sur tout ce qui a changé cette année pour financer une formation en langue en 2026 : plafond, participation, et cette exception.

Adresse en commentaire.

#CPF #TOEIC #FormationEnLangue #Anglais` },

    { angle: 'Le chiffre', texte:
`1 500 €. 150 €. Et une date : le 27 juin 2026.

Trois textes ont changé la façon de financer une formation en langue en 2026. Deux resserrent, un ouvre.

→ 1 500 € : le plafond de droits CPF mobilisables pour une certification du Répertoire spécifique, depuis le 26 février. Attention, il porte sur vos DROITS, pas sur le prix de la formation.

→ 150 € : la participation forfaitaire à l'inscription depuis le 2 avril, contre 103,20 € avant. Les demandeurs d'emploi en sont dispensés, et les salariés dont l'employeur abonde aussi.

→ 27 juin : le CPF ne finance plus une certification déjà obtenue. Sauf les certifications de langue, expressément réservées par la loi.

Chaque chiffre de cet article a son décret et sa date. Nous n'en publions pas d'autres.

Le détail est en commentaire.

#CPF #Financement #FormationProfessionnelle #Qualiopi` },

    { angle: 'Le terrain', texte:
`« On m'a dit que c'était intégralement pris en charge. »

Nous l'entendons encore chaque semaine. Or financer une formation en langue en 2026 ne se joue plus comme l'an dernier : depuis février, cette phrase est rarement exacte.

Un plafond de 1 500 € s'applique désormais aux droits CPF mobilisables pour une certification de langue. Le prix de la formation, lui, n'est pas plafonné : c'est la part payée par vos droits qui l'est.

La différence n'est pas théorique. Elle se découvre au moment de valider le dossier, et c'est le plus mauvais moment.

Notre façon de faire n'a pas changé : on regarde le solde et le coût réel AVANT d'annoncer quoi que ce soit. Puis on monte le cofinancement.

Ce qu'il faut savoir pour financer une formation en langue en 2026, texte par texte et date par date : c'est en commentaire.

#CPF #Financement #FormationEnLangue #Nice` }
  ]
};
